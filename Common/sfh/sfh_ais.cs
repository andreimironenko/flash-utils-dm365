/* --------------------------------------------------------------------------
    ...
    AUTHOR      : Stanley Park, Daniel Allred
...
namespace TIBootAndFlash
{
  ...
  public enum MagicFlags : uint
  {
    ...
    UBL_MAGIC_SPI_FLASH = 0xA1ACED01,               /* SPI option added; Got it from spiwriter.h with UBL_MAGIC_SPI24*/
  };
  ...
  partial class Program
  {
    ...
    {
      Console.Write("Usage:\n");
      Console.Write("\tsfh_ais_"+devString+" < -norerase | -nanderase > [<Options>]\n");
      Console.Write("\t\t" + "-norerase\tDo a erase of the NOR flash.\n");
      Console.Write("\t\t" + "-nanderase\tDo a erase of the NAND flash.\n");
      Console.Write("\tsfh_ais_"+devString+" -norflash_noubl [<Options>] <Application binary image>\n");
      Console.Write("\t\t" + "-norflash_noubl\tRestore NOR Flash with bootable application (typically U-Boot).\n");
      Console.Write("\tsfh_ais_"+devString+" -norflash [<Options>] <UBL binary image> <Application binary image>\n");                
      Console.Write("\t\t" + "-norflash\tRestore the NOR Flash with bootable UBL and application (typically U-Boot).\n");                 
      Console.Write("\tsfh_ais_"+devString+" -nandflash [<Options>] <UBL binary image> <Application binary image>\n");
      Console.Write("\t\t" + "-nandflash\tRestore the NAND Flash with bootable UBL and application (typically U-Boot).\n");                 
      Console.Write("\tsfh_ais_"+devString+" -sdflash [<Options>] <UBL binary image> <Application binary image>\n");      
      Console.Write("\t\t" + "-sdflash\tRestore the SD/MMC Flash with bootable UBL and application (typically U-Boot).\n");


        /*SPI helper is added*/
      Console.Write("\tsfh_ais_" + devString + " -spiflash [<Options>] <UBL binary image> <Application binary image>\n");
      Console.Write("\t\t" + "-spiflash\tRestore the SPI memory with bootable UBL and application (typically U-Boot).\n"); 
    ...
    public static ProgramCmdParams ParseCmdLine(String[] args)
    {
      ...
      myCmdParams.SerialPortBaudRate = 230400;//Serial port Baudrate for L137
        ...
        if (s.StartsWith("-"))
        {
          switch (s.Substring(1).ToLower())
          {
            ...
            case "nanderase":
              ...
              case "spiflash":
              if (myCmdParams.CMDMagicFlag == MagicFlags.MAGIC_NUMBER_INVALID)
                myCmdParams.CMDMagicFlag = MagicFlags.UBL_MAGIC_SPI_FLASH;
              else
                myCmdParams.valid = false;
              numFiles = 2;
              myCmdParams.UBLFlashType = FlashType.SPI;
              cmdString = "Flashing SPI with ";
              break;
      ...
      if (myCmdParams.UBLStartAddr == 0xFFFFFFFF)
        myCmdParams.UBLStartAddr = 0x00000100;
          //myCmdParams.UBLStartAddr = 0x80000000;
    ...
    static Int32 Main(String[] args)
    {
      ...
      workerThread.Start(); //AHA PROBLEM
    ...
    public static void WorkerThreadStart()
    {
      ...
      {      
        ...
        // May Not necessary part
      
          
          
          
          //************************************************************************************************************
        #region Place to be replaced by AIS

        if (cmdParams.UBLFlashType == FlashType.NAND)
          srchStr = "sft_"+devString+"_nand.bin";
        else if (cmdParams.UBLFlashType == FlashType.NOR)
          srchStr = "sft_"+devString+"_nor.bin";
        else 
            srchStr = "sft_" + devString + "_spi.bin";
        
        // Extract the embedded SLT
        EmbeddedFileIO.ExtractFile(System.Reflection.Assembly.GetExecutingAssembly(), srchStr, true); //Problem need to be solved for SPI
        ...
        #region AIS file maker
        Console.WriteLine("Modify UBL file to have a AIS format");
 
        Byte[] ais_MagicNum = {0x54,0x49,0x50,0x41};
          Byte[] ais_SectionLoad = { 0x01, 0x59, 0x53, 0x58 };
          Byte[] ais_Sec_addr = { 0x00, 0x00, 0x00, 0x80 }; //to L3 shared memory

          Byte[] ais_Sec_Size = new Byte[4];
          ais_Sec_Size = BitConverter.GetBytes(imageData.Length); //get imageData.Length as size to load
          
          Byte[] ais_JnC = { 0x06, 0x59, 0x53, 0x58 };
          Byte[] ais_JnC_addr = { 0x00, 0x00, 0x00, 0x80 };// to L3 shared memory
         
          Byte[] ais = new Byte[ais_MagicNum.Length + ais_SectionLoad.Length + ais_Sec_addr.Length + ais_Sec_Size.Length + imageData.Length + ais_JnC.Length + ais_JnC_addr.Length];

          foreach (byte data in ais_MagicNum)
          {
              ais[i] = data;
              i++;
          }
          foreach (byte data in ais_SectionLoad)
          {
              ais[i] = data;
              i++;
          }
          foreach (byte data in ais_Sec_addr)
          {
              ais[i] = data;
              i++;
          } foreach (byte data in ais_Sec_Size)
          {
              ais[i] = data;
              i++;
          } foreach (byte data in imageData)
          {
              ais[i] = data;
              i++;
          } foreach (byte data in ais_JnC)
          {
              ais[i] = data;
              i++;
          } foreach (byte data in ais_JnC_addr)
          {
              ais[i] = data;
              i++;
          }
          ...
          Console.WriteLine("AIS file is created");
          Console.WriteLine("Entering AIS_Parser");


          AIS_Parser parser = new AIS_Parser(MySP); // = new AIS_Parser(AIS_Parser.AIS_UART); //, this.UTIL_log, this.UTIL_uartRead, this.UTIL_uartWrite);
      ...
      BOOTUBLSEQ1:
        ...
        switch (cmdParams.CMDMagicFlag)
        {
          ...
          case MagicFlags.UBL_MAGIC_SPI_FLASH:/* added for SPI support */
          {
              status = TransmitUBLandAPP();
              break;
          }    
