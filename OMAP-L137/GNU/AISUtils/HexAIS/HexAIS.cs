/* --------------------------------------------------------------------------
    ...
    DESC        : Non-secure AIS generator for OMAP-L137/C674x
 ----------------------------------------------------------------------------- */
...
namespace TIBootAndFlash
{
  partial class Program
  {
    ...
    {
      Console.Write("Usage:\n\n");
      Console.Write("HexAIS_"+devString+"[Options] <Input File Name>\n");
    ...
    static ProgramCmdParams ParseCmdLine(String[] args)
    {
      ...
      if (fi.Exists)
      {
        ...
        if (!extension.Equals(".out",StringComparison.OrdinalIgnoreCase))
        {
          ...
          Console.WriteLine("\tIt may not be a valid COFF or ELF file.");
        }
      }
      else
      {
