...
// ----------------------------------------------------------------------- 
//                                                                         
// sfh_AIS_Util.cs -- Implements AIS_Parser class (C#) | Interaction Protocol over UART with Device
...
namespace AIS_Util
{
    ...
    class AIS_Parser
    {
		...
		/* Following is the interactions over UART to Device. 
		 * SPIFlasher() is a function to check the uart connection with target
		 * ApplicationStart() is a function to send and confirm the application to flash
		 *
		 * First, it wait for 'BOOTUBL' for check that Application is running. After that, host sends 'SUCCESS' to target, 
		 * and the target retransmits what it received back to the host. Host check two byte array and if they are matching, confirm that
		 * UART connection is working correctly on both transmiting and receiving. 
		 *
		 * Once host confirm the UART connection, it sends size of application and application image to the target over UART
		 * Target will send 'APPISHERE' to host if it received the image correctly
		 * Target will send 'DONE' when the flashing is done. Wait for the 'DONE' string and if it receives it, exit the program
		 */
		
		
		
		/**/
        public void SPIFlasher()
        {

           
            byte[] rcvEnd = new byte[8];
            byte[] corEnd = { (byte)'B', (byte)'O', (byte)'O', (byte)'T',
                                  (byte)'U', (byte)'B', (byte)'L', (byte)0 };
            byte[] corEnd2 = { (byte)'S', (byte)'U', (byte)'C', (byte)'C',
                                  (byte)'E', (byte)'S', (byte)'S', (byte)0 };
            int i = 0;
            int statu = 0;
            ...
            Console.WriteLine("Waiting for BOOTUBL...");
            statu = UTIL_uartRead(rcvEnd, 0, 8, ioTimeout); //read "BOOTUBL" from target
            ...
            for (i = 0; i < 7; i++)
            {
                if (rcvEnd[i] != corEnd[i])
                {
                    Console.WriteLine("BOOTUBL not received.");
                    break;
                }
            }
            ...
            if(statu == 0)
                Console.WriteLine("BOOTUBL received");

            statu = UTIL_uartWrite(corEnd2, 0, 8, ioTimeout); //Send 'SUCCESS'

            Console.WriteLine("\nSUCCESS sent");

            statu = UTIL_uartRead(rcvEnd, 0, 8, ioTimeout);

            
       
            for (i = 0; i < 7; i++)
            {
                if (rcvEnd[i] != corEnd2[i]) //Confirm that transmit and receive on target side is working
                {
                    Console.WriteLine("SUCCESS not received.");
                    break;
                }
                else
                    if (i == 6)
                        Console.WriteLine("SUCCESS received. UART connection correctly working");
            }
        ...
        public void ApplicationStart(byte[] app, int size, byte[] SizeByte) 
        {
            
            int status = 0;
            int i;
            byte[] rcvEnd = new byte[10];
            byte[] rcvEnd2 = new byte[5];
            byte[] test = new byte[size];
            byte[] corEnd = { (byte)'A', (byte)'P', (byte)'P', (byte)'I',
                                  (byte)'S', (byte)'H', (byte)'E', (byte)'R', (byte)'E', (byte)0 };
            byte[] corEnd1 = { (byte)'D', (byte)'O', (byte)'N', (byte)'E',
                                  (byte)0 };

            Console.WriteLine("\nSend Application size");

            status = UTIL_uartWrite(SizeByte, 0, 4, ioTimeout); // transmit size of Application to flash
            
            Console.WriteLine("\nStart Transmitting Application");
            status |= LOCAL_bufWrite(app, 0, size, ioTimeout); // Transmit Application image


            status = UTIL_uartRead(rcvEnd, 0, 10, ioTimeout); 

            for (i = 0; i < 9; i++)
            {
                if (rcvEnd[i] != corEnd[i])
                {
                    Console.WriteLine("\nAPPISHERE not received.");
                    break;
                }
                else
                    if (i == 8)
            ...
            status = UTIL_uartRead(rcvEnd2, 0, 5, ioTimeout); //read "DONE"

            for (i = 0; i < 4; i++)
            {
                if (rcvEnd2[i] != corEnd1[i])
                {
                    Console.WriteLine("\nDONE not received.");
                    ...
                    {
                        Console.WriteLine("\nDONE received");
                        Console.WriteLine("SPI Flashing Completed");
                    }
